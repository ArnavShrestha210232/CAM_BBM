...
typedef struct
{
  int camera_id;
  const char *uart_dev;
  const char *uart_dev2; /* Secondary UART interface (e.g. /dev/ttyS4) */
  int spi_bus;
...
/* Transmits byte to primary UART and optional secondary UART */
...
static void *camera_worker_thread(void *arg)
{
  ...
  int cam_idx = id - 1; /* 0-based array index for ring buffer */
...
int camera_MSN2_main(int argc, char *argv[])
{
  ...
  camera_ctx_t cam1_ctx = {
      .camera_id = 1,
      .uart_dev = "/dev/ttyS2",
      .uart_dev2 = "/dev/ttyS4", /* Duplicates all Camera 1 TX data over UART 4 */
      .spi_bus = 3,
  ...
  camera_ctx_t cam2_ctx = {
      .camera_id = 2,
      .uart_dev = "/dev/ttyS3",
      .uart_dev2 = NULL, /* Single UART output */
      .spi_bus = 4,
  ...
  sem_wait(&g_subscriber_done_sem); /* Wait for Camera 1 ring buffer dump */
  sem_wait(&g_subscriber_done_sem); /* Wait for Camera 2 ring buffer dump */
